...
public class Menu extends CommonHandler {
	...
	public void drawMenuBar() {
		...
		// If the top level menu category item has changed then redraw the whole menu bar.
	 ...
	 * which handles such things as moving around with the arrows keys,
	 * selected an option with the ENTER key or ESCaping out of the menu 
	 * system.
	 * 
	...
	public void processKeyEvent(KeyEvent e) {
		...
		if (key == KeyEvent.VK_ESCAPE) {
			if (menuOption == null) {
				// This is the ESC that activated the menu, so start with ABOUT selected.
				menuOption = MenuOption.ABOUT;
	...
	public void processMouseEvent(int eventType, Point mousePoint) {
		...
		if (eventType == MouseEvent.MOUSE_PRESSED) {
			if (!editStatus.isMenuActive()) {
				// If the menu isn't yet active then this is the mouse click that should activate the menu system.
				editStatus.setMenuActive(true);
			} else {
				// Close menu system before responding to selection.
				closeMenuSystem();
	...
	private void processMenuSelection(MenuOption menuOption) {
		if (menuOption.equals(MenuOption.NEWPIC)) {
			...
			int answer = JOptionPane.showOptionDialog(application, "Are you sure you want to create a new picture?", "", JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE, null, options, options[0]);
		...
		if (menuOption.equals(MenuOption.QUIT)) {
			...
			int answer = JOptionPane.showOptionDialog(application, "Are you sure you want to Quit?", "", JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE, null, options, options[0]);
			if (answer == JOptionPane.YES_OPTION) {
			    System.exit(0);
			}
		}
